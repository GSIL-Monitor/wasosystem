        ...
        $traverse = function ($DivisionalManagement, $prefix, $parent_id, $year, $mouth) use (&$traverse) {
            foreach ($DivisionalManagement as $category) {
                ...
                if ($category->identifying == 'company') {
                    ...
                    echo '<dt>公司：</dt>';
                    echo '<dd>';
                    echo ' <a data_url="' . $url . '?parent_id=' . $category->id . '&year=' . $year . '&mouth=' . $mouth . '" class="task_managements" data-id="">' . $category->name . '</a>';
                    echo '</dd>';
                    ...
                    if ($category->children->isNotEmpty()) {
                        foreach ($category->children as $cate) {
                            echo ' <a data_url="' . $url . '?parent_id=' . $cate->id . '&type=' . $cate->identifying . '&year=' . $year . '&mouth=' . $mouth . '" class="list_' . $cate->id . ' task_managements">' . $cate->name . '</a>';
                        }
                    }
                    echo '</dd>';
                    echo ' <div class="clear"></div>';
    ...
    public function chart_tree($data, $parent_id, $year, $mouth)
    {
        ...
        foreach ($cates as $key => $item) {
            ...
            foreach ($arr[$key] as $key2 => $item2) {
                if ($key2 == $parent_id) {
                    echo ' <a href="' . route('admin.task_managements.marketing_statistics') . '?year=' . $year . '&mouth=' . $mouth . '&parent_id=' . $key2 . '" class="active">' . $item2 . '</a>';
                } else {
                    echo ' <a href="' . route('admin.task_managements.marketing_statistics') . '?year=' . $year . '&mouth=' . $mouth . '&parent_id=' . $key2 . '" >' . $item2 . '</a>';
                }
...
//            echo '<tr style="color:' . randomColor() . '!important;">';
//            echo '<td>' . $key . '</td>';
//            echo '<td>' . $item . '</td>';
//            echo '</tr>';
        }
            ...
            foreach ($DivisionalManagement as $category) {
                if ($category->admins && $category->task) {
                    if ($category->admins->demand->isNotEmpty()) {
                        ...
                        $outstanding[$category->admins->account] = self::outstanding($category->admins->order);
                        $guaranteed_task[$category->admins->account] = self::calculation($category->task->guaranteed_task);
...
//                    dump($returned_money,$monthly_sales,$outstanding,$RewardsAndPunishment);
    ...
    //当月销售
    public static function monthly_sales($order)
    {
    ...
    //发出未结
    public static function outstanding($order)
    {
...
//回款未结
    ...
    static function receivable_outstanding($returned_money, $outstanding, $guaranteed_task)
    {
        ...
        if ($returned_money_sum && $guaranteed_task_sum && $outstanding_sum) {
