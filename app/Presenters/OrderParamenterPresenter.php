...
use DB;
class OrderParamenterPresenter
{
    ...
    //获取旧订单物料
    public function get_goods($order_product_goods,$order)
    {
        $TerraceOrMainboard=$this->CheckTerraceOrMainboard($order_product_goods);//平台或者主板
        $crate=$order_product_goods->firstWhere('product_id','=',20) ?? '';//机箱
        //判断有无阵列卡
        $raid=$order_product_goods->firstWhere('product_id','=',17) ?? '';
        ...
        foreach ($order_product_goods as $key=>$product_good){
            if($product_good->product_id == 23 ){//平台
                ...
                if($product_good->product_id == 12){
                    ...
                    $cpu_num=$product_good->pivot->product_good_num;
    ...
    public function checkAdiator($product_good,$order)
    {
        ...
        $PlatformRadiator['terrace_price']=$product_good->price[$order->user->grades->identifying] ?? 0; //平台价格
        if($goods) {
    ...
    public function checkProduct($product_good,$TerraceOrMainboard,$terraceNum){
        ...
        if(str_contains($product_good->product_id, [13,20,21,23]) || $product_good->product_id == 22 && $terraceNum== 0){
            ...
            $arr['add_symbol']='+';
            $arr['del_symbol']='-';
            $arr['del_button']='<i class="Del" d data_title="'.$product_good->name.'" data-id="'.$product_good->id.'"></i><input class="selectBox selectIds" type="checkbox" name="id[]" value="'.$product_good->id.'">';
            $arr['id']=$product_good->id;
            $arr['readonly']='';
        }
        if($product_good->pivot->product_good_num ==1){
            $arr['del_class']='none';
            $arr['del_symbol']='';
...
//                $arr['del_button']='&nbsp;';
//            }else{
//                $arr['del_button']='<input class="selectBox selectIds" type="checkbox" name="id[]" value="'.$product_good->id.'">';
//            }
            $arr['readonly']='';
        }
       return $arr;
    ...
    public function cpu($product_good){
        ...
        $filtered=json_filter($filtered,'zui_da_pei_zhi', '>=', $product_good->details['cpu_cha_cao_shu'] ?? $product_good->details['jie_dian_cpu_bing_cun_shu'])->pluck('name','id');
    ...
    public function memory($product_good,$cpu_num,$node_number){
        ...
        $list=$product_good
           ->where($where)
            ...
            ->oldest('name')
            ->get();
        $filtered=json_filter($list,'rong_liang', '<=', $rong_liang)->pluck('name','id');
