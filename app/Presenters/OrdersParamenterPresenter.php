...
use DB;
class OrdersParamenterPresenter
{
    ...
    public static function get_good_parameter($good)
    {
        $data=[];
        ...
        $data['list']=$good->product->good->pluck('name','id')->toArray();
        $user_product=$good->user_product ?? collect([]);
        return array_merge($data,$user_product->toArray());
    }
    //获取旧订单物料
    public function get_goods($order_product_goods)
    {
        ...
        foreach ($product_goods as $key=>$product_good){
            ...
            $pgd=array_merge(self::get_good_parameter($product_good),$other);
            if($product_good->product_id == 23 ){//平台
                $arr[$key]=$pgd;
                $terraceNum=$product_good['user_product']->product_good_num;//参数
            }else{ //主板
                $arr[$key]=$pgd;
                if($product_good->product_id == 12){
                    $arr[$key]=array_merge($arr[$key],$this->cpu($TerraceOrMainboard));
                    $cpu_num=$product_good['user_product']->product_good_num;//参数
       ...
       return $a->toJson(JSON_UNESCAPED_UNICODE);
    }
...
//            if($product_good->product_id ==15 && $product_good['user_product']->product_good_num ==1){ //如果硬盘只有一个 不显示删除按钮
//                $arr['del_button']='&nbsp;';
//            }else{
...
//            }
    ...
    public function cpu($product_good){
         ...
         $cpu['num']=$product_good->user_product->product_good_num;
        ...
        $filtered=json_filter($filtered,'zui_da_pei_zhi', '>=', $product_good->details['cpu_cha_cao_shu'] ?? $product_good->details['jie_dian_cpu_bing_cun_shu'])->pluck('name','id')->toArray();
    ...
    public function memory($product_good,$cpu_num,$node_number){
        ...
        $list=$product_good
           ->where($where)
            ...
            ->oldest('name')
            ->get();
        $filtered=json_filter($list,'rong_liang', '<=', $rong_liang)->pluck('name','id')->toArray();
        //筛选
