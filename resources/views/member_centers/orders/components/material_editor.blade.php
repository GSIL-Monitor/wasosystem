...
<div class="editBoxWrap" >
    <div class="editBox">
        ...
        <div class="editInfo">
            <div class="DoneInfo">
                <div class="headPic radius"><img src="{{ order_complete_machine_pic($user_products) }}"></div>
                <div class="editName">
                    <form id="completeMachine">
                        {!!  Form::hidden('null',$order->id,["class"=>'order_id']) !!}
                        {!!  Form::hidden('null',1,["class"=>'order_num']) !!}
                        {!!  Form::hidden('price_spread',$order->price_spread ?? 0,["class"=>'price_spread']) !!}
                        <h5 class="xinghao"> {!!  Form::text('machine_model',$order->machine_model,["class"=>'xh name','readonly']) !!}</h5>
                        {!!  Form::hidden('code',$order->code,["class"=>'codes']) !!}
                        <input type="hidden" value="{{ config('site.order_type')[$order->order_type]  }}" class="code" />
                        {!!  Form::hidden('order_type',$order->order_type) !!}
