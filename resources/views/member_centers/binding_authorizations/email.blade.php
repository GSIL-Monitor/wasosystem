<dd>
    <div class="nowInfo" v-if="!edit_email && !new_email">
        ...
        <span class="name">
                                            <h5> @if(!user()->email)未绑定 @else {{ user()->email }} @endif</h5>
                                            <h6> @if(!user()->email)绑定后可以通过邮箱找回密码 @else 已验证，可通过邮箱找回密码 @endif</h6>
                                    </span>
        <span class="control">
                            @if(!user()->email)
            ...
            <div class="editBox">
                <ul class="safeUl">
                    ...
                    <li :class="{ errorBorder: errors.has('email_code') }">
                        ...
                        <a class="repSend canSend" @click="send('{{ route('binding_authorization.send') }}','{{ user()->email }}')">@{{content}}</a></li>
                </ul>
                <div class="button goStep2" @click="check_code('{{ route('binding_authorization.check_code') }}','email_code')">确 认</div>
            ...
            <div class="editBox">
                <ul class="safeUl">
                    ...
                    <li :class="{ errorBorder: errors.has('email_code') }">
                        ...
                        <div class="vee_error" v-show="errors.has('email_code')"><i></i>
                            <p>@{{ errors.first('email_code') }}</p></div>
                        <a class="repSend canSend" @click="send('{{ route('binding_authorization.send') }}',email)">@{{content}}</a></li>
                </ul>
                <div class="button goStep3" @click="bind('{{ route('binding_authorization.bind') }}',email,'email')">确 认</div>
                <div class="button cancel"  @click="cancel('email')">取 消</div>
                <div class="clear"></div>
            </div>
        </div>
