    ...
    <div class="step1" v-show="!next_step">
        <ul name="mobile" class="tab">
            ...
            <li  :class="{ errorBorder: errors.has('email_code') }">
                ...
                <a class="repSend canSend" @click="send('{{ route('binding_authorization.send') }}',email,'email')">@{{content}}</a></li>

        </ul>
        <div class="button goStep2" @click="next('{{ route('binding_authorization.check_code') }}',email,'email')"><i class="wait"><img src="{{ asset('pic/wait.gif') }}"></i><a>下一步</a></div>
    </div>
    <!-- 第一步完成  -->
