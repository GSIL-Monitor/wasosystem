...
@include('admin.common.field._input',['title'=>'借出时间','field'=>null,'field_val'=>$barcode_associated['warehouse_out_management']->created_at,'other'=>['class'=>'checkNull','readonly']])
{!! Form::hidden('user_id',$barcode_associated['warehouse_out_management']->user->id) !!}
@include('admin.common.field._input',['title'=>'关联客户','field'=>null,'field_val'=>$barcode_associated['warehouse_out_management']->user->username,'other'=>['class'=>'checkNull','readonly']])
@include('admin.common.field._input',['title'=>'当前事件','field'=>null,'field_val'=>config('status.barcode_associateds_type')[$barcode_associated['warehouse_out_management']->out_type],'other'=>['class'=>'checkNull','readonly']])
@include('admin.common.field._select',['title'=>'选择事件','field'=>'current_state','field_list'=>array_only(config('status.barcode_associateds_type'),['borrow_to_sales','loan_out_return']),'other'=>['placeholder'=>'请选择事件','class'=>'checkNull','v-model'=>'selected','@change'=>'changeSelect()']])
@include('admin.common.field._input',['title'=>'经办人','field'=>null,'field_val'=>$order->markets->name ?? $admin_id[$barcode_associated['warehouse_out_management']->user->administrator],'other'=>['class'=>'checkNull','readonly']])
@include('admin.common.field._select',['title'=>'产品成色','field'=>'product_colour','field_list'=>array_only(config('status.barcode_associateds_type'),['new','good','bad']),'other'=>['placeholder'=>'请选择产品成色','class'=>'checkNull',':disabled'=>'showColor']])
...
@include('admin.common.field._textarea',['title'=>'借出备注','field'=>null,'field_val'=>$barcode_associated['warehouse_out_management']->postscript,'other'=>['readonly']])
