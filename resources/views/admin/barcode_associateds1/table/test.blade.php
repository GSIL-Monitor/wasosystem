        ...
        @forelse($barcode_associateds as $key=>$barcode_associated)
            ...
            <tr>
                ...
                <td class="tableInfoDel  tablePhoneShow  tableName"><a class="changeWeb"
                                                                       data_url="{{ route('admin.barcode_associateds.create') }}?status={{ $barcode_associated['barcode_associateds']->procurement_type }}&id={{ $barcode_associated['barcode_associateds']->id }}{{ array_to_url(array_except($barcode_associated,'barcode_associateds')) }}">{{ $key }}</a>
                </td>
