...
<head lang="en">
    ...
    <div class="IE">
        <div class="logo_bg">
            <a class="logo" href=""><img src="{{ asset('admin/pic/logo.png') }}"></a>
        </div>
        <div class="tips">
            <p>很抱歉，您目前的浏览器版本较低</p>
            ...
            <div class="container">
                ...
                <div class="login">
                    ...
                    <div class="Down radius">
                        ...
                        <div class="checkPic"><img title="点击刷新" id="che_pic" src="{!! captcha_src('waso') !!}" align="absbottom" onClick="this.src=this.src+'?'+Math.random()" style="cursor: pointer"></div>
                        <div class="clear"></div>
                    </div>
                    ...
                    <div class="loginNow"><b class="">登 录</b><img src="{{ asset('admin/pic/loading.gif') }}"></div>
                    {{ Form::close() }}
                </div>
            </div>
            <div class="Loginfoot">Copyright © <span class="time">{{ date('Y',time()) }}</span></br>网烁信息科技有限公司 版权所有</div>
        </div>
    </div>
    ...
    $(document).ready(function () {
        ...
        function Pic() {
            ...
            {{--if(winW<=900){--}}
                {{--$("video").remove();--}}
            {{--}else{--}}
                {{--$(".bgPic").append("<video autoplay loop muted  preload='auto' ><source src='{{ asset("admin/video/login.mp4") }}'></video>");--}}
            {{--}--}}
        }
        ...
        $(document).on("click", '.loginNow', function () {
            ...
            axios.post(
                ...
                .then(function (response) {
                        location.href="{{ route('admin.waso') }}";
