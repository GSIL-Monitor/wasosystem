    ...
    <ul class="maxUl"  >
        ...
        @endif
                    ...
                    <table class="listTable">
                        ...
                        @endif
                            ...
                            <tr>
                                ...
                                <td class="num">
                                        <input type="text" readonly="" class="PJnum good_num" style="text-align: center;padding: 0" value="{{ $product_good->pivot->product_good_num  }}"  product-name="{{ $product_good->product->title }}"  product-bianhao="{{  $product_good->product->bianhao }}" good-id="{{ $product_good->id }}" good-framework="{{ $product_good->framework->name }}" good-jianma="{{ $product_good->jianma }}" >
                                </td>
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="addPro" id="app">
                                        ...
                                        <input class="red AllDel Btn" data_url="{{ url('/waso/complete_machines/destory') }}?goodDel={{ $complete_machine->id ?? 'admins' }}&complete_machine_id={{ $complete_machine->id ?? Auth::user()->id }}"  type="button" value="删除">
            ...
            <li class="sevenLi">
                ...
                <div class="liRight">
                    ...
                    @foreach(config('status.complete_machine_prices') as $key=>$value)
                        @if($key=='balance')
                            <label class="priceLabel"><div class="priceTit">{{ $value }}：</div><div class="priceCont">{!!  Form::number('price['.$key.']',old('price['.$key.']',$complete_machine->price[$key] ?? 0),['placeholder'=>'请输入'.$value,'class'=>'checkNull','id'=>$key,'original_price'=>$productGood->price[$key] ?? 0]) !!}</label>
                        @else
            ...
            <li>
                ...
                <div class="liRight">
                    {!!  Form::number('quality_time',old('quality_time'),['placeholder'=>'整机质保(年)','class'=>'checkNull']) !!}
                </div>
                <div class="clear"></div>
            </li>
            ...
            <li>
                ...
                <div class="liRight">
                    {!!  Form::number('weight',old('weight'),['placeholder'=>'重量(Kg)']) !!}
                </div>
                <div class="clear"></div>
            </li>
            <li>
