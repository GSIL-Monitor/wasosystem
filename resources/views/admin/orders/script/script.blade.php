...
//                        this.set_order_satus_true();
    ...
    @endif
        ...
        $(document).on('click','.contract_alert',function () {
            swal({
                title: '{{ $order->serial_number }}合同模板',
                width: 700,
                ...
                location.href="{{ route('admin.orders.export',$order->id) }}?view={{ $view }}&export=Contract&"+form
            }).catch(swal.noop)
        })
        $(document).on('click','.system_solution_alert',function () {

            var company='{{ $company_name }} {{ $order->user->nickname }}';
            var user_remark="{{ str_replace(array("\r\n", "\r", "\n"),'',$order->user_remark) }}";
            var company_remark="{{ str_replace(array("\r\n", "\r", "\n"),'',$order->company_remark) }}";
            swal({
                title: '{{ $order->serial_number }}整机方案书',
                width: 700,
                allowOutsideClick:false,
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                ...
                location.href="{{ route('admin.orders.export',$order->id) }}?view=system_solution&export=SystemSolution&"+form
            }).catch(swal.noop)
        })
        ...
        $(document).on('click','.signature_form_alert',function () {
            swal({
                ...
                location.href="{{ route('admin.orders.export',$order->id) }}?view=signature_form&export=SignatureForm&type=complete_machine";
            },function () {
                location.href="{{ route('admin.orders.export',$order->id) }}?view=signature_form&export=SignatureForm&type=parts";
            })
        })
