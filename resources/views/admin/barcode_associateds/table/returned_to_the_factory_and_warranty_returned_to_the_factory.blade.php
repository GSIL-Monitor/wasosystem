 <form>
    <table class="listTable">
        <tr>
            ...
            <th class="">操作员</th>
            <th class="">入库时间</th>
            <th class="tableMoreHide">受理时间</th>
        </tr>
        ...
        @forelse($barcode_associateds as $barcode_associated)
            ...
            <tr>
                ...
                <td class="tableInfoDel  tablePhoneShow  tableName"><a class="changeWeb"
                                                                       data_url="{{ route('admin.barcode_associateds.create') }}?status={{ $barcode_associated->current_state }}&id={{ $barcode_associated->id }}&code={{ $barcode_associated->code }}&product_good_id={{ $barcode_associated->product_good->id }}"">{{ $barcode_associated->code }}</a>
                </td>
                ...
                <td class="">{{ $barcode_associated->procurement_plans->updated_at->format('Y-m-d') }}</td>
                <td class="tableMoreHide">{{ $barcode_associated->updated_at->format('Y-m-d') }}</td>
            </tr>
        @empty
