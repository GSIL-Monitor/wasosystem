        ...
        @forelse($barcode_associateds as $barcode_associated)
            ...
            <tr>
                ...
                <td class="tableInfoDel  tablePhoneShow  tableName"><a class="changeWeb"
                                                                       data_url="{{ route('admin.barcode_associateds.create') }}?status={{ $status }}&id={{ $barcode_associated->id }}&code={{ $barcode_associated->code }}&product_good_id={{ $barcode_associated->product_good->id }}"">{{ $barcode_associated->code }}</a>
                </td>
