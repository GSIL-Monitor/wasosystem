...
@section('js')
    <script>
        function pay(serial_number,price,type) {
            axios.post("{{ route('admin.funds_managements.pay') }}", {
                "_token": getToken(),
                "user_id": "{{ $user->id }}",
                "type": type,
        ...
        $(function () {
            $(document).on('click', '.deposit', function () {
                ...
                swal({
                    ...
                    location.href = "{{ route('admin.funds_managements.deposit') }}?user_id={{ $user->id }}&" + form
                }).catch(swal.noop)
            });
    ...
    <div class="nowWebBox">
        <div class="PageBtn">
            ...
            <div class="PageBtnTxt">
              ...
              <div>站内可用：<span class="Internal_funds">{{ $Internal_funds }}</span> / 未付款：<span class="non-payment">{{ $orders->sum('total_prices') }}</span></div>
            </div>
        </div>
        <div class="PageBox">
            @include('admin.funds_managements.form')
        </div>
    </div>
