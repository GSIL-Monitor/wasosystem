    ...
    <div>
        <div class="demo-upload-list " v-for="item in uploadList" :key="item.name" v-dragging="{ item: item, list: uploadList, group: 'item' }" >
            <div  style="display: none" >
                <Input type="text" name="file[url][]"  :value="item.url_name"></Input>
            </div>
            ...
            <div class="pic_name">
                <Input type="text" style="width: 100px;"  name="file[name][]" :value="item.name"></Input>
            </div>
        </div>
        <!--</transition-group>-->
        <!--</draggable>-->
        ...
        <Modal title="View Image" v-model="visible">
            <img :src="pic" v-if="visible" style="width:100%">
        </Modal>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handleFormatError (file) {
                this.$Notice.warning({
                    title: '文件格式不正确',
                    desc: '文件格式 ' + file.name + ' 不正确，请选择zip,rar或mp4 ?.'
                });
            },
            handleMaxSize (file) {
                this.$Notice.warning({
            ...
            handleBeforeUpload () {
                ...
                if (!check) {
                    this.$Notice.warning({
                        title: '最多可以上传'+this.fileCount+'个文件'
                    });
                }
                return check;
            }
