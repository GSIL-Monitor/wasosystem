...
class CreateOrdersTable extends Migration {
	...
	public function up()
	{
		Schema::create('orders', function(Blueprint $table)
		{
			...
			$table->boolean('flow_pic')->default(0)->comment('是否流程图');
			$table->boolean('in_common_use')->default(0)->comment('是否常用配置');
			$table->text('pic')->nullable()->comment('装机流程图');
			$table->integer('market')->index()->comment('销售人员');
			$table->text('participation_admin')->nullable()->comment('订单参与人员');
