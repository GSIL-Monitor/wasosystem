...
class CreateCompleteMachineFrameworksTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('complete_machine_frameworks', function (Blueprint $table) {
            ...
            $table->enum('category',['framework','Application','answer','filtrate','product'])->default('framework')->index()->comment('参数分类');
            $table->json('pic')->nullable()->comment('图片');
            $table->enum('select_type',['checkbox','radio'])->nullable()->default('radio')->comment('筛选类型');
    ...
    {
        Schema::dropIfExists('complete_machine_frameworks');
    }
}
